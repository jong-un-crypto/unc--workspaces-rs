...
use unc_workspaces::{Account, AccountId, Contract, Worker};
use unc_workspaces::{BlockHeight, DevNetwork};
use serde_json::json;

const FT_CONTRACT_FILEPATH: &str = "./examples/res/fungible_token.wasm";

/// Contract id of ref-finance on mainnet.
const REF_FINANCE_ACCOUNT_ID: &str = "v2.ref-finance.unc";
...
async fn create_ref(owner: &Account, worker: &Worker<Sandbox>) -> anyhow::Result<Contract> {
    let mainnet = unc_workspaces::mainnet_archival().await?;
    ...
    owner
        ...
        .args_json(json!({}))
        .deposit(UncToken::from_millinear(30))
        .transact()
        .await?
        .into_result()?;
...
async fn create_wnear(owner: &Account, worker: &Worker<Sandbox>) -> anyhow::Result<Contract> {
    let mainnet = unc_workspaces::mainnet_archival().await?;
    let wunc_id: AccountId = "wrap.unc".to_string().try_into()?;
    let wnear = worker
        .import_contract(&wunc_id, &mainnet)
        .block_height(BLOCK_HEIGHT)
        .transact()
        .await?;
    ...
    owner
        ...
        .args_json(json!({}))
        .deposit(UncToken::from_millinear(8))
        .transact()
        .await?
        .into_result()?;

    owner
        .call(wnear.id(), "unc_deposit")
        .deposit(UncToken::from_unc(200))
        .transact()
        .await?
        .into_result()?;
...
async fn create_pool_with_liquidity(
    ...
    let pool_id: u64 = ref_finance
        ...
        .deposit(UncToken::from_millinear(3))
        .transact()
        .await?
        .json()?;
    ...
    owner
        ...
        .deposit(UncToken::from_yoctounc(1))
        .transact()
        .await?
        .into_result()?;
...
async fn main() -> anyhow::Result<()> {
    ...
    let pool_id = create_pool_with_liquidity(
        ...
        &ref_finance,
        maplit::hashmap! {
            ft.id() => UncToken::from_unc(5).a.as_yoctounc(),
            wnear.id() => UncToken::from_unc(10).a.as_yoctounc(),
        },
    )
    .await?;
    println!(
    ...
    deposit_tokens(
        ...
        &ref_finance,
        maplit::hashmap! {
            ft.id() => UncToken::from_unc(100).a.as_yoctounc(),
            wnear.id() => UncToken::from_unc(100).a.as_yoctounc(),
        },
    )
    .await?;
    ...
    let ft_deposit: UncToken = worker
    ...
    assert_eq!(ft_deposit, UncToken::from_unc(100));

    let wunc_deposit: UncToken = worker
        .view(ref_finance.id(), "get_deposit")
        .args_json(json!({
            "account_id": owner.id(),
            "token_id": wnear.id(),
    ...
    println!("Current WNear deposit: {}", wunc_deposit);
    assert_eq!(wunc_deposit, UncToken::from_unc(100));
    ...
    assert_eq!(ft_deposit, UncToken::from_unc(99));

    let wunc_deposit: String = ref_finance
        .view("get_deposit")
        .args_json(json!({
            "account_id": owner.id(),
            "token_id": wnear.id(),
        ...
        .await?
        .json()?;
    println!("New WNear deposit after swap: {}", wunc_deposit);
