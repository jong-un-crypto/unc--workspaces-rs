...
use unc_primitives::borsh::{self, BorshDeserialize, BorshSerialize};
use unc_token::UncToken;
use serde_json::json;
use test_log::test;

use unc_workspaces::types::{KeyType, SecretKey};
use unc_workspaces::{AccessKey, AccountDetailsPatch, AccountId, Contract, DevNetwork, Worker};

const STATUS_MSG_WASM_FILEPATH: &str = "../examples/res/status_message.wasm";

#[derive(Clone, Eq, PartialEq, Debug, BorshDeserialize, BorshSerialize)]
#[borsh(crate = "unc_primitives::borsh")]
struct Record {
    k: String,
    v: String,
}
...
async fn test_patch_full() -> anyhow::Result<()> {
    ...
    let bob_id: AccountId = "bob.test.unc".parse()?;
...
async fn test_patch_account_from_current() -> anyhow::Result<()> {
    ...
    const NEW_BALANCE: UncToken = UncToken::from_yoctounc(10_u128.pow(16));

    let f = |mut acc: unc_workspaces::types::AccountDetails| {
