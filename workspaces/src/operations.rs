...
use crate::types::{
    AccessKey, AccountId, Gas, InMemorySigner, KeyType, UncToken, PublicKey, SecretKey,
};
    ...
    pub fn call(mut self, function: Function) -> Self {
        ...
        if let Ok(actions) = &mut self.actions {
            actions.push(Action::FunctionCall(Box::new(FunctionCallAction {
                ...
                args,
                deposit: function.deposit.as_yoctounc(),
...
impl CallTransaction {
    ...
    /// Specify the amount of gas to be used where `gas` is the amount of gas in yocto unc.
    ...
    pub async fn transact_async(self) -> Result<TransactionStatus> {
        send_batch_tx_async_and_retry(
            ...
            vec![FunctionCallAction {
                ...
                deposit: self.function.deposit.as_yoctounc(),
            }
            .into()],
        )
        .await
...
impl<'a, 'b> CreateAccountTransaction<'a, 'b> {
    pub(crate) fn new(
        ...
        Self {
            ...
            parent_id,
            new_account_id,
            initial_balance: UncToken::from_yoctounc(100000000000000000000000u128),
            secret_key: None,
        }
    }
...
///
/// [`asynchronous transaction`]: https://docs.unc.org/api/rpc/transactions#send-transaction-async
#[must_use]
