...
use unc_jsonrpc_client::methods::query::RpcQueryResponse;
use unc_jsonrpc_client::methods::{self, RpcMethod};
use unc_jsonrpc_primitives::types::chunks::ChunkReference;
use unc_jsonrpc_primitives::types::query::QueryResponseKind;
use unc_primitives::types::{BlockId, BlockReference, StoreKey};
use unc_primitives::views::{BlockView, QueryRequest};
...
impl<'a, T> Query<'a, T> {
    ...
    pub fn block_hash(mut self, hash: CryptoHash) -> Self {
        self.block_ref = Some(BlockId::Hash(unc_primitives::hash::CryptoHash(hash.0)).into());
        self
    }
}
...
/// Trait used as a converter from WorkspaceRequest to unc-rpc request, and from unc-rpc
...
pub trait ProcessQuery {
    ...
    /// the type from unc-primitives to a workspace type.
    type Output;
    ...
    /// to workspaces related types from the unc-primitives or json types from the network.
...
impl Query<'_, ViewFunction> {
    ...
    pub fn args_borsh<U: unc_primitives::borsh::BorshSerialize>(mut self, args: U) -> Self {
...
impl ProcessQuery for GasPrice {
    ...
    fn from_response(resp: <Self::Method as RpcMethod>::Response) -> Result<Self::Output> {
        Ok(UncToken::from_yoctounc(resp.gas_price))
    }
}
...
impl<'a> QueryChunk<'a> {
    ...
    pub fn block_hash_and_shard(mut self, hash: CryptoHash, shard_id: ShardId) -> Self {
        self.chunk_ref = Some(ChunkReference::BlockShardId {
            block_id: BlockId::Hash(unc_primitives::hash::CryptoHash(hash.0)),
            shard_id,
        });
        self
    }
    ...
    pub fn chunk_hash(mut self, hash: CryptoHash) -> Self {
        self.chunk_ref = Some(ChunkReference::ChunkHash {
            chunk_id: unc_primitives::hash::CryptoHash(hash.0),
        });
        self
    }
}
