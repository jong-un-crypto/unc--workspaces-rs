...
/// how and where it can be used, visit [Gas](https://docs.unc.org/docs/concepts/gas).
...
impl PublicKey {
    ...
    fn try_from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        Ok(Self(
            unc_crypto::PublicKey::try_from_slice(bytes).map_err(|e| {
                ErrorKind::DataConversion
                    .full(format!("Invalid key data for key type: {key_type}"), e)
            })?,
        ))
...
impl InMemorySigner {
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        let signer = unc_crypto::InMemorySigner::from_file(path)
...
pub struct FunctionCallPermission {
    ...
    // values for this field (see: https://github.com/unc/nearcore/pull/4621#issuecomment-892099860)
                ...
                AccessKeyPermission::FunctionCall(function_call_permission) => {
                    unc_primitives::account::AccessKeyPermission::FunctionCall(
                        unc_primitives::account::FunctionCallPermission {
                            allowance: function_call_permission.allowance.map(|a| a.as_yoctounc()),
