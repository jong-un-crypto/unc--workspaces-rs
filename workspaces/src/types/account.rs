...
use crate::types::{AccountId, InMemorySigner, UncToken, PublicKey, SecretKey};
...
impl AccountDetailsPatch {
    ...
    pub fn locked(mut self, locked: UncToken) -> Self {
        self.locked = Some(locked);
        self
    }
...
impl AccountDetails {
    ...
    pub(crate) fn into_unc_account(self) -> unc_primitives::account::Account {
        unc_primitives::account::Account::new(
            self.balance.as_yoctounc(),
            self.locked.as_yoctounc(),
            0,
...
impl From<AccountView> for AccountDetails {
    fn from(account: AccountView) -> Self {
        Self {
            balance: UncToken::from_yoctounc(account.amount),
            locked: UncToken::from_yoctounc(account.locked),
