...
async fn acquire_unused_port() -> Result<(u16, File)> {
    loop {
        ...
        let lockpath = std::env::temp_dir().join(format!("unc-sandbox-port{}.lock", port));
...
async fn init_home_dir() -> Result<TempDir> {
    init_home_dir_with_version(sandbox::DEFAULT_UNC_SANDBOX_VERSION).await
}
...
impl SandboxServer {
    ...
    pub(crate) async fn run_new() -> Result<Self> {
        Self::run_new_with_version(sandbox::DEFAULT_UNC_SANDBOX_VERSION).await
    }
