...
async fn test_contract_transfer() -> anyhow::Result<()> {
    let transfer_amount = UncToken::from_millinear(100);
    let worker = unc_workspaces::sandbox().await?;
...
- `UNC_SANDBOX_BIN_PATH`: Set this to our own prebuilt `neard-sandbox` bin path if we want to use a non-default version of the sandbox or configure nearcore with our own custom features that we want to test in unc-workspaces.
